...
def get_friends_similarity(uid1, uid2, dataset):
    ...
    if total_fruiends == 0:
        return 0
    return common_friends / total_fruiends
    
def get_implicit_neighbor_for_single(uid, rate_uid_iid_dict, rate_iid_uid_dict, feat_iid_fid_dict, feat_fid_iids_dict, dataset):
    """
    :param uid:
        ...
        for iid in uid_iid_dict[uid]:
            ...
            for feat_name in feat_iid_fid_dict.keys():
                ...
                for s_iid, s_prob in s_iid_prob.items():
                    ...
                    for k, v in uid_prob.items():
                        #add power user score
                        power_weight = get_friends_similarity(uid, k, dataset)
...
def get_training_samples(dataset_path, user2id, item2id, user_feature, item_feature, output_dir, **kwargs):
    ...
    for state in states:
        ...
        for _, (user, items) in tqdm(enumerate(dataset.items())):
            ...
            uid = user2id[user]
            # print(".....user id:", user)
            # print("len(items), len(labels):", (len(items), len(labels)))
            ...
            # if item_len < 13 or item_len > 23: # modify for yelp 
            if item_len < 13 or item_len > 50:
                continue
            # if state == "warm_up":
            #     print("generate support for user.....", user)
    ...
    for state in states:
        ...
        for _, (sup_x, sup_y) in tqdm(enumerate(zip(support_x_list, support_y_list))):
            ...
            for coclick in coclick_list:
                if coclick not in meta_training_feat_dict:
                    continue
