#!/Users/lujiewen/Desktop/papers/PAML-base//python

# /Users/lujiewen/miniconda3/bin/python dataset.py
import datetime
import pandas as pd
...
class yelp(object):
    def __init__(self):
        self.rating_data, self.user_feature, self.item_feature, self.rating_list,self.user_friends = self.load()
        self.user_neighbor = self.get_neighbor()
    ...
    def load(self):
        ...
        print("user_features: ", user_feature)
        print('user in user_feature: %s' % len(user_feature['user'].unique()))
        ...
        return rating_data, user_feature, item_feature, rating_list, user_friends
    ...
    def get_neighbor(self):
        ...
        with open("PAML/proposed_model_for_yelp/data/yelp/original/user_friends.dat") as fin:
            for line in fin:
                ...
                user2s = list(map(int, data[1].split(' '))) #modified for yelp
