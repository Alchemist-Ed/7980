...
class MAML(torch.nn.Module):
    def __init__(self, config):
        ...
        # global update
        # setting outler loop learning rate, usually smaller than inner loop lr, which is local lr
        self.meta_optimizer = torch.optim.NAdam(self.parameters(), lr=config['outer_lr'], weight_decay=config['weight_decay'])
        self.time_spend = {}
    ...
    def local_update(self, task_data):
        ...
        grad = torch.autograd.grad(loss, task_weights.values(), create_graph=True)
    ...
    def global_update(self, global_step, batch_data):
        ...
        # global update
        start_time = time.time()
        grads = torch.autograd.grad(total_loss, self.parameters())
        with torch.no_grad():
