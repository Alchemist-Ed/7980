...
Fig. 1. The framework of the Preference-Adaptive Meta-Learning method for cold-start recommendation (adapted from Fig.1 in [1]).
...
Fig. 2. MAML loss function explanation (adapted from Fig.2 in [2]).
...
Fig. 3. Meta-learning pipeline using implicit gradients (adapted from Fig.1 in [2]).
...
Fig. 4. Meta-learning pipeline using implicit gradients (adapted from [4]).
...
Fig. 5. Graphical structure of coarse-fine preference modeling. (adapted from Fig.3 in [1]).
...
Fig. 6. Overview of the enhanced iMAML model
...
Fig. 7. Ttraining time cost by different models graph (adapted from [7]).
...
Fig. 8.nDCG@5 by different models graph (adapted from [7]).
